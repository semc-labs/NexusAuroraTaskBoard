...
module.exports = {
		...
		await interaction.reply("SHIB is currently at: **"+json.data.prices.latest+"** USD\n\nThat places NA's ~~.4~~ .2 billion SHIB at: **"+formatNums(round(Number(json.data.prices.latest)*Number(process.env.TOTAL_SHIB), 2))), 2)+"% "+(percentIncrease(11401.40, Number(json.data.prices.latest)*Number(process.env.TOTAL_SHIB)) > 0 ? "Up" : "Down") + " from its original value** USD or "+round(Math.abs(percentIncrease(11401.40, Number(json.data.prices.latest)*Number(process.env.TOTAL_SHIB))+"\n\n**We have sold 50% of our $SHIB, the command now shows the updated value**");
	},
};
